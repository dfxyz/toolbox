...
pub const NAME: &str = "hide";

#[inline]
pub fn args<'a>() -> clap::Command<'a> {
    clap::Command::new(NAME)
        .about("Hide the given file(s).")
        .trailing_var_arg(true)
        .arg(clap::arg!(-h --help "Print help information."))
        .arg(clap::arg!(-s --system "Also set the \"system\" attribute."))
        .arg(clap::arg!(<FILE>... "File(s) to hide."))
}
...
pub fn run_with_matches(matches: &clap::ArgMatches) -> ! {
    let set_system_attr = matches.is_present("system");
    matches.values_of("FILE").unwrap().for_each(|f| {
        ...
        unsafe {
            ...
            if SetFileAttributesW(file, attr) == 0 {
                eprintln!("ERROR: failed to hide \"{}\".", f);
            }
        }
    });
    std::process::exit(0);
